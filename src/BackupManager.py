...
#
	...
	def Stage6(self, result=None, retval=None, extra_args=None):
		...
		if self.didPluginsRestore or self.didSettingsRestore:
			...
			self.Console.ePopen("sleep 100" + " &&" + " /sbin/init 4" + " &&" + " sleep 5" + " &&" + " tar -xzvf" + " " + self.BackupDirectory + self.sel + " -C /" + " &&" + " rm -f /tmp/ExtraInstalledPlugins /tmp/backupimageversion /tmp/backupkernelversion" + " && /sbin/init 3 && sleep 15 && /sbin/init 6", self.Stage1SettingsComplete, self.session.open(MessageBox, _("Restoring please wait..."), MessageBox.TYPE_INFO))
		else:
